...
productRouter.get(
  ...
  expressAsyncHandler(async (req, res) => {
    ...
    const raitingFilter = rating ? { rating: { $gte: rating } } : {};
    const sortOrder =
      order === 'lowest'
        ? { price: 1 }
        : order === 'highest'
  ...
  expressAsyncHandler(async (req, res) => {
    ...
    if (product) {
      if (product.reviews.find((x) => x.name === req.user.name)) {
        return res.status(400).send({ message: 'You already submitted a product review' });
      }
      const review = {
        name: req.user.name,
      ...
      product.rating =
        product.reviews.reduce((accumalater, currently) => currently.rating + accumalater, 0) /
